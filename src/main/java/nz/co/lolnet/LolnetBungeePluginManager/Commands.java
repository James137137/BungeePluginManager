...
public class Commands extends Command {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "list": {
                sender.sendMessage(textWithColor("Plugin [" + ProxyServer.getInstance().getPluginManager().getPlugins().size() + "]:", ChatColor.GREEN));
                String messageToSend = "";
                for (Plugin plugin : ProxyServer.getInstance().getPluginManager().getPlugins()) {
                    if (messageToSend.length() == 0) {
                        messageToSend += plugin.getDescription().getName();
                    } else if ((messageToSend + plugin.getDescription().getName()).length() >= 100) {
                        sender.sendMessage(textWithColor(messageToSend, ChatColor.GREEN));
                        messageToSend = "";
                    } else {
                        messageToSend += ", " + plugin.getDescription().getName();
                    }
